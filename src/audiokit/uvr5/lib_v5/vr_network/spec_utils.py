...
def wave_to_spectrogram_mt(
    ...
    def run_thread(**kwargs):
        spec_left.append(librosa.stft(**kwargs))
    ...
    spec = np.asfortranarray([spec_left[0], spec_right])
...
def spectrogram_to_wave_mt(spec, hop_length, mid_side, reverse, mid_side_b2):
    ...
    def run_thread(**kwargs):
        wave_left.append(librosa.istft(**kwargs))
    ...
    if reverse:
        return np.asfortranarray([np.flip(wave_left[0]), np.flip(wave_right)])
    elif mid_side:
        return np.asfortranarray(
            [np.add(wave_left[0], wave_right / 2), np.subtract(wave_left[0], wave_right / 2)]
        )
    elif mid_side_b2:
        return np.asfortranarray(
            [
                np.add(wave_right / 1.25, 0.4 * wave_left[0]),
                np.subtract(wave_left[0] / 1.25, 0.4 * wave_right),
            ]
        )
    else:
        return np.asfortranarray([wave_left[0], wave_right])
