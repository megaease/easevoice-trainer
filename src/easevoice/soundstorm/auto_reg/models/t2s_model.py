...
class T2SBlock:
    ...
    @torch.jit.ignore  # pyright: ignore
...
class Text2SemanticDecoder(nn.Module):
    ...
    def forward(self, x, x_lens, y, y_lens, bert_feature):
        ...
        loss_2, _, _ = dpo_loss(A_logits, R_logits, 0, 0, 0.2, reference_free=True)  # pyright: ignore
    ...
    def infer(
        ...
        for _ in tqdm(range(1500)):
            ...
            if stop:
                ...
                break
            # current generated semantic_ids and previous y to make new y
    ...
    def infer_panel_batch_infer(
        self,
        x: Union[List[torch.LongTensor], Any],
        ...
        **kwargs,
        ...
        x_mask = F.pad(
            x_attn_mask,
            (0, y_len),  # xx zeros extend to xx zeros + xy ones, (x,x+y)
            value=True,
        )
        y_mask = F.pad(  # yy right up 1 extend to left xy 0, (y, x+y)
            torch.triu(torch.ones(y_len, y_len, dtype=torch.bool), diagonal=1),
            (x_len, 0),
            value=False,
        )
        ...
        ###### decode #####
        y_list: Any = [None]*y.shape[0]
        batch_idx_map = list(range(y.shape[0]))
        idx_list: Any = [None]*y.shape[0]
        for idx in tqdm(range(1500)):
            if idx == 0:
                xy_dec, k_cache, v_cache = self.t2s_transformer.process_prompt(xy_pos, xy_attn_mask, xy_padding_mask, False)
            else:
                xy_dec, k_cache, v_cache = self.t2s_transformer.decode_next_token(xy_pos, k_cache, v_cache, xy_attn_mask, False)  # pyright: ignore
            ...
            # remove finished data from batch, optim calculations
            tokens = torch.argmax(logits, dim=-1)
            reserved_idx_of_batch_for_y = None
            if (self.EOS in samples[:, 0]) or \
                    (self.EOS in tokens):  # stop when meet EOS
            ...
            # only keep not finish ones in batch
            ...
            xy_pos = y_emb * self.ar_audio_position.x_scale + self.ar_audio_position.alpha * self.ar_audio_position.pe[:, y_len + idx].to(dtype=y_emb.dtype, device=y_emb.device)  # pyright: ignore

        if (None in idx_list):
            for i in range(x.shape[0]):
                if idx_list[i] is None:
                    idx_list[i] = 1500-1  # if not generate EOS, use max length instead
    ...
    def infer_panel_naive_batched(self,
                                  x: List[torch.LongTensor],  # all text tokens
                                  x_lens: torch.LongTensor,
                                  prompts: torch.LongTensor,  # audio tokens
                                  bert_feature: List[torch.LongTensor],
                                  top_k: int = -100,
                                  top_p: int = 100,
                                  early_stop_num: int = -1,
        ...
        for i in range(len(x)):
            y, idx = self.infer_panel_naive(x[i].unsqueeze(0),  # pyright: ignore
                                            x_lens[i],  # pyright: ignore
                                            prompts[i].unsqueeze(0) if prompts is not None else None,  # pyright: ignore
                                            bert_feature[i].unsqueeze(0),  # pyright: ignore
                                            top_k,
                                            top_p,
                                            early_stop_num,
                                            temperature,
    ...
    def infer_panel_naive(
        self,
        x: torch.LongTensor, # all text tokens
        x_lens: torch.LongTensor,
        prompts: torch.LongTensor, # audio tokens
        ...
        **kwargs
        ...
        x_attn_mask_pad = F.pad(
            x_attn_mask,
            (0, y_len),  # xx zeros to xx zeros and xy ones (x, x+y)
            value=True,
        )
        y_attn_mask = F.pad(  # yy right top ones to left xy zeros (y, x+y)
            torch.triu(torch.ones(y_len, y_len, dtype=torch.bool), diagonal=1),
            (x_len, 0),
            value=False,
        )
        ...
        for idx in tqdm(range(1500)):
            if xy_attn_mask is not None:
                ...
                xy_dec, k_cache, v_cache = self.t2s_transformer.decode_next_token(xy_pos, k_cache, v_cache)  # pyright: ignore
            ...
            if idx == 0:
                xy_attn_mask = None
            if (idx < 11):  # at least 10 tokens otherwise not stop
                logits = logits[:, :-1]
        ...
        if ref_free:
            return y[:, :-1], 0
        return y[:, :-1], idx - 1  # pyright: ignore

    def infer_panel(
        self,
        x: torch.LongTensor, # all text token
        x_lens: torch.LongTensor,
        prompts: torch.LongTensor,  # audio tokens
        bert_feature: torch.LongTensor,
        top_k: int = -100,
