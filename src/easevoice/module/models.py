import contextlib
from torch.cuda.amp import autocast  # pyright: ignore
...
class Generator(torch.nn.Module):
    def __init__(
        ...
        self.conv_post = Conv1d(ch, 1, 7, 1, padding=3, bias=False)  # pyright: ignore
        ...
        for i in range(self.num_upsamples):
            ...
            for j in range(self.num_kernels):
                if xs is None:
                    ...
                    xs += self.resblocks[i * self.num_kernels + j](x)
            x = xs / self.num_kernels  # pyright: ignore
        x = F.leaky_relu(x)
        x = self.conv_post(x)
        x = torch.tanh(x)
