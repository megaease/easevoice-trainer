...
from torch.nn.utils import weight_norm, remove_weight_norm  # pyright: ignore

from src.easevoice.module import commons
from src.easevoice.module.commons import init_weights, get_padding
from src.easevoice.module.transforms import piecewise_rational_quadratic_transform
    ...
    def infer(self, inputs=None, random_sample=False, manual_latent=None):
        if manual_latent is None:
            if random_sample:
                ...
                enc_out = self.ref_encoder(inputs.transpose(1, 2))  # pyright: ignore
                mu = self.fc1(enc_out)
                z = mu
        else:
            z = manual_latent
