...
import requests
import zipfile
from ..chinese_norm.char_convert import tranditional_to_simplified
...
class G2PWOnnxConverter:
    def __init__(self,
        ...
        self.config = load_config(
            config_path=os.path.join(uncompress_path, 'config.py'),  # pyright: ignore
            use_default=True)
        ...
        for sent_id, sent in enumerate(sentences):
            ...
            for i, char in enumerate(sent):
                if char in self.polyphonic_chars_new:
                    ...
                    partial_result[i] = pypinyin_result[i][0]  # pyright: ignore
                else:
                    partial_result[i] = pypinyin_result[i][0]  # pyright: ignore
