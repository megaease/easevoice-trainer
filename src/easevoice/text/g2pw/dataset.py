...
def prepare_onnx_input(tokenizer,
    ...
    for idx in range(len(texts)):
        text = (truncated_texts if window_size else texts)[idx].lower()  # pyright: ignore
        query_id = (truncated_query_ids if window_size else query_ids)[idx]  # pyright: ignore
        ...
        position_id = text2token[query_id] + 1  # [CLS] token locate at first place # pyright: ignore
...
def _truncate(max_len: int,
    ...
    end = token2text[token_end - 1][1]  # pyright: ignore
    ...
    ], [(s - start, e - start) for s, e in token2text[token_start:token_end]])  # pyright: ignore
